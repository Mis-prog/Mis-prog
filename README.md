...
- 🎓 M.Sc. in Math & CS (Machine Learning track) @ Central University — previously B.Sc. Applied Math @ Ufa State Aviation Technical University
- 🧠 Interests: RAG architectures, LLM agents, information retrieval, MLOps

**Tech stack**

`PyTorch` `Transformers` `LangChain` `LangGraph` `vLLM` `SGLang` `Qdrant` `FAISS`
`CatBoost` `XGBoost` `LightGBM` `OpenCV` `PySpark` `Airflow` `ClickHouse` `Docker` `CI/CD`

📫 [Telegram](https://t.me/mihail_akmurzin) · akmurzinmihail@gmail.com
